...
import lstm_crf_data_helper
import lstm_crf_model
...
# 用于每个字标记向索引的映射，注意用于实体识别时，“O”对应的必须是0因为标签的填充是以0进行填充的。

def getalllabel():  # 自动从训练集和测试集中找到所有的类别标签
    with open('data/train_data', 'r', encoding='utf-8') as f1:
        corpus1 = [str(i).strip('\n') for i in f1.readlines() if str(i).strip('\n') != '']
    with open('data/test_data', 'r', encoding='utf-8') as f2:
        corpus2 = [str(i).strip('\n') for i in f2.readlines() if str(i).strip('\n') != '']
    corpus = corpus1 + corpus2
    label = [j.split('\t')[1] for j in corpus]
    label = set(label)
    labels = {}
